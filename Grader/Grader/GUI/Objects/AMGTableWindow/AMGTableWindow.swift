...
class AMGTableWindow : NSWindow, NSTableViewDelegate, NSTableViewDataSource{
    ...
    ///   - session2: Grader instance to grab the details from
    ///   - table: Name of the table to grab the details from
    public init(frame2 : NSRect, session2 : AMGGrader , table : String){
    ...
    public init(frame2 : NSRect, session2 : AMGGrader , table : String, window : String){
        super.init(contentRect: frame2, styleMask: [.closable, .borderless, .resizable, .titled], backing: .buffered, defer: false);
        session = session2;
        tableName = table;
        windowName = window;
