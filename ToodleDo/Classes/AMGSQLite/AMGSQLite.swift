        ...
        if(self.connect()){
            do{
                ...
                if(sqlite3_prepare_v2(databaseHandler, sql , -1, &queryHandler, nil) == SQLITE_OK){
                    while(sqlite3_step(queryHandler) == SQLITE_ROW){
                        ...
                        for i in 0 ..< sqlite3_column_count(queryHandler){
							...
							if(sqlite3_column_text(queryHandler, i) != nil){
								currentRow = currentRow.appending(String.init(cString: sqlite3_column_text(queryHandler, i)));
							}
							else{
								currentRow = currentRow.appending(String(format:"%s",sqlite3_column_value(queryHandler, i)));
							}
                        }
                    ...
                    if(errorTable != ""){
                        safeQuery(sql: String(format: "insert into %@ (sql_text,error_text) values ('%@','%@')", errorTable,sql,String(cString: sqlite3_errmsg(queryHandler), encoding: .utf8)!));
                    }
            ...
            do{
                if(sqlite3_prepare_v2(databaseHandler, sql, -1, &queryHandler, nil) == SQLITE_OK){
                    if(sqlite3_step(queryHandler) == SQLITE_DONE){
                        ...
                        if(errorTable != ""){
                            safeQuery(sql: String(format: "insert into %@ (%@_command_text,%@_error_text) values ('%@','%@')",
												  errorTable,errorTable,errorTable,sql,String(cString: sqlite3_errmsg(queryHandler), encoding: .utf8)!));
                        }
                    }
                }
                else{
